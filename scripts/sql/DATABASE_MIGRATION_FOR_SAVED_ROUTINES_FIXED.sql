-- Database Migration for Saved Routines Feature (FIXED VERSION)
...
-- 2. Add saved_routine_id column to routine table (without AFTER clause - safer)
ALTER TABLE `routine` 
...
-- 5. Drop old unique constraint (check actual name first with: SHOW INDEX FROM routine;)
-- If this fails, check the actual constraint name in phpMyAdmin
ALTER TABLE `routine` 
